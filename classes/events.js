...
class Events {
	...
	parse() {
		...
		this.bot.on('messageCreate', async function(discord) {
			...
			if (message.startsWith(Config.cmdchar)) {
				try {
					...
					if (command.server && channel.type !== 'text') return channel.say("This command is only available in servers.");
		...
		this.bot.on('messageReactionAdd', (reaction, user) => {
			...
			if (!Db('emojis').has(emoji.name)) Db('emojis').set(emoji.name, 1);
			if (user.bot) return;
			if (emoji.name === emote.left && reaction.count > 1) return Client.shop.change(1);
			if (emoji.name === emote.right && reaction.count > 1) return Client.shop.change(2);
		});
